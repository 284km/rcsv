...
class TestCSV::Encodings < TestCSV
  ...
  def test_read_with_default_encoding
    ...
    each_encoding do |encoding|
      ...
      begin
        ...
        result = Rcsv.read(@temp_csv_path)[0][0]
      ensure
        no_warnings do
  ...
  def test_auto_line_ending_detection
    ...
    encode_for_tests([["a" * 509]], row_sep: "\r\n") do |data|
      assert_equal("\r\n".encode(data.encoding), Rcsv.new(data).row_sep)
    end
  end
    ...
    encode_for_tests([%w[abc def]]) do |data|
      %w[col_sep row_sep quote_char].each do |csv_char|
        assert_equal( "|".encode(data.encoding),
                      # Rcsv.new(data, csv_char.to_sym => "|").send(csv_char) )
                      Rcsv.new(data, csv_char.to_sym => "|").send(csv_char) )
      end
    end
  end
  ...
  #     parsed = Rcsv.parse(data, headers: true)
  ...
  #   encode_for_tests([%w[one two three], %w[1 2 3]]) do |data|
  #     parsed = Rcsv.parse(data, converters: :integer)
  ...
  #     Rcsv.open(@temp_csv_path, "rb:#{data.encoding.name}") do |csv|
  #       csv.each do |row|
  ...
  #       f << data
  #     end
  #     Rcsv.open(@temp_csv_path, "rb:UTF-32BE:#{data.encoding.name}") do |csv|
  #       csv.each do |row|
  #         assert_all?(row, "Wrong data encoding.") {|f| f.encoding == data.encoding}
  #       end
  #     end
  ...
  #     Rcsv.foreach(@temp_csv_path, encoding: data.encoding) do |row|
  ...
  #     # read and write in encoding
  #     File.open(@temp_csv_path, "wb:#{data.encoding.name}") { |f| f << data }
  #     rows = Rcsv.read(@temp_csv_path, encoding: data.encoding.name)
  ...
  def test_can_write_csv_in_any_encoding
    each_encoding do |encoding|
      ...
      Rcsv.open(@temp_csv_path, "wb:#{encoding.name}") do |f|
        data.each { |row| f << row }
      end
      assert_equal(data, Rcsv.read(@temp_csv_path, encoding: encoding.name))
    end
  end
  ...
  def test_explicit_encoding
    ...
    s = Rcsv.generate(encoding: "Windows-31J") do |csv|
  ...
  def assert_parses(fields, encoding, options = { })
    ...
    parsed   = Rcsv.parse(data, options)
    ...
    Rcsv.open(@temp_csv_path, "rb:#{encoding}", options) do |csv|
      csv.each_with_index do |row, i|
        assert_equal(fields[i], row)
      end
    end
    begin
      Rcsv.open(@temp_csv_path, "rb:#{encoding}:#{__ENCODING__}", options) do |csv|
        csv.each_with_index do |row, i|
    ...
    Rcsv.open(@temp_csv_path, options) do |csv|
      csv.each_with_index do |row, i|
        assert_equal(fields[i], row)
      end
    end
