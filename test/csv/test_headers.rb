...
class TestCSV::Headers < TestCSV
  ...
  def test_first_row
    [:first_row, true].each do |setting|  # two names for the same setting
      ...
      assert_nothing_raised(Exception) do
        csv = Rcsv.parse(@data, headers: setting)
      end
  ...
  def test_array_of_headers
    ...
    csv = nil
    assert_nothing_raised(Exception) do
      csv = Rcsv.parse(@data, headers: [:my, :new, :headers])
    end
  ...
  def test_csv_header_string_inherits_separators
    ...
    csv = nil
    assert_nothing_raised(Exception) do
      csv = Rcsv.parse( @data.tr(",", "|"), col_sep: "|",
  ...
  def test_return_headers
    ...
    csv = nil
    assert_nothing_raised(Exception) do
      csv = Rcsv.parse(@data, headers: true, return_headers: true)
    end
  ...
  def test_converters
    ...
    csv = Rcsv.parse( data, headers:        true,
                           return_headers: true,
                           converters:     :numeric )
  ...
  def test_builtin_downcase_converter
    csv = Rcsv.parse( "One,TWO Three", headers:           true,
                                      return_headers:    true,
  ...
  def test_builtin_converters_with_blank_header
    csv = Rcsv.parse( "one,,three", headers:           true,
                                   return_headers:    true,
  ...
  def test_skip_blanks
    ...
    expected = [%w[1 2 3]]
    Rcsv.parse(@data, headers: true, skip_blanks: true) do |row|
  ...
  def test_headers_reader
    # no headers
    assert_nil(Rcsv.new(@data).headers)

    # headers
    csv = Rcsv.new(@data, headers: true)
  ...
  def test_blank_row_bug_fix
    ...
    Rcsv.parse(@data, headers: true) do |row|
      assert_instance_of(CSV::Row, row)
    end
  end
end
