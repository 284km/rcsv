...
class TestCSV::Row < TestCSV
  ...
  def setup
    super
    @row = Rcsv::Row.new(%w{A B C A A}, [1, 2, 3, 4])
  end

  def test_initialize
    # basic
    row = Rcsv::Row.new(%w{A B C}, [1, 2, 3])
    assert_not_nil(row)
    assert_instance_of(Rcsv::Row, row)
    assert_equal([["A", 1], ["B", 2], ["C", 3]], row.to_a)

    # missing headers
    row = Rcsv::Row.new(%w{A}, [1, 2, 3])
    assert_not_nil(row)
    assert_instance_of(Rcsv::Row, row)
    assert_equal([["A", 1], [nil, 2], [nil, 3]], row.to_a)

    # missing fields
    row = Rcsv::Row.new(%w{A B C}, [1, 2])
    assert_not_nil(row)
    assert_instance_of(Rcsv::Row, row)
  ...
  def test_row_type
    # field rows
    row = Rcsv::Row.new(%w{A B C}, [1, 2, 3])         # implicit
    assert_not_predicate(row, :header_row?)
    assert_predicate(row, :field_row?)
    row = Rcsv::Row.new(%w{A B C}, [1, 2, 3], false)  # explicit
    assert_not_predicate(row, :header_row?)
    assert_predicate(row, :field_row?)

    # header row
    row = Rcsv::Row.new(%w{A B C}, [1, 2, 3], true)
  ...
  def test_to_a
    row = Rcsv::Row.new(%w{A B C}, [1, 2, 3]).to_a
  ...
  def test_inspect_shows_symbol_headers_as_bare_attributes
    str = Rcsv::Row.new(@row.headers.map { |h| h.to_sym }, @row.fields).inspect
    @row.each do |header, field|
  ...
  def test_can_be_compared_with_other_classes
    assert_not_nil(Rcsv::Row.new([ ], [ ]), "The row was nil")
  end
  ...
  def test_dig_cell
    row = Rcsv::Row.new(%w{A}, [["foo", ["bar", ["baz"]]]])
