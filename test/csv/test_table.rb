...
class TestCSV::Table < TestCSV
  ...
  def setup
    super
    @rows  = [ Rcsv::Row.new(%w{A B C}, [1, 2, 3]),
               Rcsv::Row.new(%w{A B C}, [4, 5, 6]),
               Rcsv::Row.new(%w{A B C}, [7, 8, 9]) ]
    @table = Rcsv::Table.new(@rows)

    @header_table = Rcsv::Table.new(
      [Rcsv::Row.new(%w{A B C}, %w{A B C}, true)] + @rows
    )
  end

  def test_initialze
    assert_not_nil(@table)
    assert_instance_of(Rcsv::Table, @table)
  end
  ...
  def test_set_row_or_column
    ...
    @table[3] = Rcsv::Row.new(%w[A B C], [13, 14, 15])
  ...
  def test_set_by_col_with_header_row
    r  = [ Rcsv::Row.new(%w{X Y Z}, [97, 98, 99], true) ]
    t = Rcsv::Table.new(r)
    t.by_col!
    t['A'] = [42]
    assert_equal(['A'], t['A'])
  ...
  def test_each
    ...
    @table.each { |row| assert_instance_of(Rcsv::Row, row) }
  ...
  def test_append
    ...
    # Array append
    assert_equal(Rcsv::Row.new(%w[A B C], [10, 11, 12]), @table[-1])

    # Row append
    assert_equal(@table, @table << Rcsv::Row.new(%w[A B C], [13, 14, 15]))
    assert_equal(Rcsv::Row.new(%w[A B C], [13, 14, 15]), @table[-1])
  end
  ...
  #   data = "col1,col2\nra1,ra2\n\nrb1,rb2"
  #   table = Rcsv.parse(data, :headers => true)
  ...
  def test_dig_cell
    table = Rcsv::Table.new([Rcsv::Row.new(["A"], [["foo", ["bar", ["baz"]]]])])
  ...
  def test_dig_cell_no_dig
    table = Rcsv::Table.new([Rcsv::Row.new(["A"], ["foo"])])
