...
class TestCSV::DataConverters < TestCSV
  ...
  def test_builtin_all_nested_combo_converter
    ...
    @parser =  Rcsv.new(@data)                  # reset parser
  ...
  def test_convert_with_custom_code_using_field_info_header
    @parser = Rcsv.new(@data, headers: %w{one two three four five})
  ...
  def test_custom_converter_with_blank_field
    ...
    row = nil
    assert_nothing_raised(Exception) do
      row = Rcsv.parse_line('nil,', converters: converter)
    end
  ...
  def test_shortcut_interface
    assert_equal( ["Numbers", ":integer", 1, ":float", 3.015],
                  Rcsv.parse_line(@data, converters: :numeric) )

    assert_equal( ["Numbers", ":integer", 1, ":float", 3.015],
                  Rcsv.parse_line(@data, converters: [:integer, :float]) )

    assert_equal( ["Numbers", :integer, 1, :float, 3.015],
                  Rcsv.parse_line(@data, converters: [:numeric, @custom]) )
  end
  ...
  # def test_nil_value
  #   assert_equal(["nil", "", "a"],
  #                Rcsv.parse_line(',"",a', nil_value: "nil"))
  # end

  # TODO:
  # def test_empty_value
  #   assert_equal([nil, "empty", "a"],
  #                Rcsv.parse_line(',"",a', empty_value: "empty"))
  # end
end
